    ...
    <div class="h-screen w-screen duration-500">
      <div class="absolute z-40 h-screen w-screen border border-gray-500 bg-blue-900 p-6 text-gray-200 duration-500 md:w-1/2 lg:w-44">
        <div class="min-w-xl z-40 overflow-y-auto overflow-x-hidden text-left text-lg mt-20">
          ...
          <SidebarLink to="/admin/summary" icon="pi pi-list-check">Invoice</SidebarLink>
        </div>
      </div>
    </div>
  </div>
  <div class="fixed z-[100] h-screen w-max cursor-pointer md:hidden">
    <div
      :class="[
        ...
        isOpen ? 'top-6 ml-6 place-items-center px-2 text-blue-800 md:pt-3' : 'top-1/4 bg-gradient-to-l from-blue-900 to-blue-600 pb-4 shadow-xl backdrop-blur-sm md:px-3 md:pb-6 md:pt-3',
      ]"
      @click="toggleSidebar"
    >
      <div>
    ...
    <div :class="['h-screen duration-500', isOpen ? 'h-screen w-screen' : 'w-0']">
      ...
      >
        <div class="z-50 mt-12 py-5 md:mt-24">
          <input type="text" class="p-2 w-full rounded bg-gray-900 text-blue-800" placeholder="Search..." />
        </div>
        <div class="min-w-xl z-40 h-4/6 overflow-y-auto overflow-x-hidden text-left text-lg">
          <SidebarLink to="/admin" icon="pi pi-home">Alat</SidebarLink>
          <SidebarLink to="/admin/all-operator" icon="pi pi-users">Teknisi</SidebarLink>
          <SidebarLink to="/admin/summary" icon="pi pi-list-check">Invoice</SidebarLink>
        </div>
